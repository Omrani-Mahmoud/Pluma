...
function CustomTextArea({action,type,v,length=200}) {
    ...
    return (
        ...
        <TextareaAutosize aria-label="minimum height" rowsMin={10} maxLength={400} style={{width:'100%',resize:'none',marginTop:'15px'}} defaultValue={v?.lenght>0?v:inputs?inputs[type]:''}  onChange={(e)=>action({type:type,value:e.target.value})}/>
