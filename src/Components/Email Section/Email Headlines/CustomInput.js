 ...
 function CustomInput({name,placeholder,action,margin=10,type,v='',length=20}) {
    ...
    return (
       <Grid item md={12} xs={12} style={{marginBottom:`${margin}px`}}>
            ...
            <TextField className={classes.txtInput}  inputProps={{maxLength:length}} error={hasError} helperText={`Max input size is ${length} character`} fullWidth placeholder={placeholder} InputLabelProps={{shrink: true}} style={{marginTop:'5px'}} defaultValue={v.lenght>0?v:inputs[type]} onChange={(e)=>check_(e)}/>
